...
2. 위 링크를 Chromium 계열 브라우저에서 엽니다. **데스크톱은 Chrome/Edge 최신 버전**, **모바일은 Android Chrome**에서 Web Bluetooth를 지원합니다. 로컬에서 테스트할 때는 `index.html`을 직접 열지 말고 `npx http-server . -p 8080` 같은 정적 서버로 `http://localhost:8080`에서 접속하세요.
...
5. 첫 데이터가 오기 전에는 웹앱이 보정 안내를 표시합니다. 보정이 끝나면 측정이 시작되고, 화면의 카드·차트·로그가 0.1초 마다 자동 갱신되며, 하단 `CSV 다운로드` 버튼으로 기록을 저장할 수 있습니다.
...
- **연결됨인데 데이터가 없음**: micro:bit LED에 나침반 보정 화면이 보이면 모든 방향으로 천천히 돌려 보정을 완료하세요. 첫 데이터가 계속 오지 않으면 연결 해제 후 다시 연결합니다.
- **갑자기 연결이 끊김**: 웹앱이 3초 정도 기다린 뒤 이전 micro:bit로 자동 재연결을 한 번 시도합니다. 실패하면 micro:bit가 재부팅된 뒤 다시 연결하세요.
