...
│   ├── bluetooth.js    # Web Bluetooth 헬퍼 (동적 특성 탐색 및 재연결 보강 포함)
...
└── docs/
    ...
    ├── connection-reliability-improvement.md # 연결 안정성 분석 및 개선안
    └── AGENT.md # 완료된 개선 작업과 후속 과제 정리
```
...
3. 연결 직후 첫 샘플이 도착하기 전에는 보정 안내가 표시됩니다. micro:bit LED에 나침반 보정 화면이 보이면 모든 방향으로 천천히 돌려 보정을 완료합니다.
4. 첫 데이터가 수신되면 실시간 카드, 차트, 로그 테이블에서 자기장 데이터를 확인하고, 필요 시 CSV를 내려받습니다.
5. 연결 해제 시 **연결 해제** 버튼을 누르거나 micro:bit 전원을 분리합니다. 예기치 않게 연결이 끊기면 앱은 짧게 대기한 뒤 이전 장치로 자동 재연결을 한 번 시도합니다.

## 5. 모듈 상세 설명

### 5.1 핵심 스크립트 (`js/` 루트)
- **app.js**: DOMContentLoaded 시점에 UI 모듈을 초기화하고 정리 함수(cleanup)를 등록합니다. `?mock=1` 파라미터를 읽어 모의 텔레메트리 활성화 여부를 결정합니다.
- **bluetooth.js**: Web Bluetooth 연결 전 과정을 담당합니다. Nordic UART 서비스를 검색하고, 특성 권한을 분석해 TX(쓰기)와 RX(알림) 채널을 자동으로 선택합니다. 수신 패킷을 조립하는 내부 버퍼(`rxBuffer`)와 연결 해제 이벤트 처리 로직을 포함합니다. 예기치 않은 해제 후 이전 장치 객체로 재연결할 수 있도록 GATT disconnect listener를 재등록하는 방어 로직도 포함합니다.
- **dialog-polyfill.js**: 브라우저가 `<dialog>`를 지원하지 않을 때 접근 가능한 모달을 제공하도록 백드롭·포커스 트랩·Esc 종료 동작을 구현합니다.
- **state.js**: 간단한 pub/sub 스토어를 구현해 연결 상태, 최신 샘플, 히스토리, 안내 메시지(`noticeMessage`)를 관리합니다. UI 모듈이 상태를 구독해 변경을 반영합니다.
- **mockTelemetry.js**: 실제 장치 없이 UI를 시험할 때 사용합니다. `?mock=1`이면 주기적으로 가짜 샘플을 생성해 상태 스토어에 전달합니다.

### 5.2 UI 모듈 (`js/ui/`)
- **connection-panel.js**: 연결/해제 버튼, 상태 표시, 안내 메시지, 오류 메시지, 마지막 수신 시간 UI를 관리하며 Bluetooth 동작과 직접 상호작용합니다. 첫 샘플 수신 전에는 `waiting-data` 상태와 보정 안내를 표시하고, 7초 동안 데이터가 없으면 조치 안내를 보여 줍니다. 예기치 않은 연결 해제 시 3초 후 이전 장치로 자동 재연결을 한 번 시도합니다. 지원 브라우저 안내 버튼이 눌리면 `<dialog id="supported-browsers-dialog">`를 열고, 폴리필을 사용해 포커스 복귀와 백드롭 클릭 닫기 처리를 담당합니다.
- **usage-guide.js**: 헤더의 `사용 안내` 버튼과 `<dialog id="usage-guide-dialog">`를 연결합니다. 모달 열기/닫기, 백드롭 클릭 닫기, 닫기 후 포커스 복귀를 처리합니다.
